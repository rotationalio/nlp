...
func TestIsStopWordEnglish(t *testing.T) {
	...
	for _, word := range stopwords.English {
		require.True(t, stopwords.IsStopWord(word, language.English), "stop word not recognized")
	}

	// different case and spacing tests
	for _, word := range []string{"about", "ABOUT", "About", " ABOUT ", "AbOuT\n", "\n \t about \n \t"} {
		require.True(t, stopwords.IsStopWord(word, language.English), "stop word not recognized in uppercase")
	}

	// false tests
	for _, word := range []string{"careful", "zebra", "aardvark", "89u3dr4wuf7y8fy7t4h8", "RACECAR", "about1234", " !@#$%^&*() \n \t"} {
		require.False(t, stopwords.IsStopWord(word, language.English), "this is not a stop word")
	}
}
